document.addEventListener('DOMContentLoaded', () => {
  ...
  function filterAndSortProducts() {
    ...
    const sortPriceOrder = sortPriceSelect.value;
    const sortStockOrder = sortStockSelect.value;
    ...
    produitItems.forEach(item => {
      ...
      // Si searchTerm est vide, afficher tout
    ...
    // Tri par prix
    if (sortPriceOrder && visibleItems.length > 0) {
    ...
    // Tri par stock
    if (sortStockOrder && visibleItems.length > 0) {
      visibleItems.sort((a, b) => {
        const stockA = parseInt(a.dataset.stock, 10);
        const stockB = parseInt(b.dataset.stock, 10);
        return sortStockOrder === 'asc' ? stockA - stockB : stockB - stockA;
      });
    }

    // Réorganiser le DOM après le tri
...
// Événements en temps réel
if (sortStockSelect) {
  sortStockSelect.addEventListener('change', () => {
      console.log('Tri par stock:', sortStockSelect.value);
      filterAndSortProducts();  // Appeler la fonction de filtrage après sélection
  });
}
